...
Training happens at a point in time, and thus the "knowledge" gained is static and may not reflect the current state of the world and would not include any private information.
...
Temperature is a fixed parameter, but its practical impact is subjective to the model due to differences in design and training between models.
